...
export default function AppDiagnostico() {
  ...
  // --- MAGIA WEBHID BLINDADA ---
  const conectarMultimetroUSB = async () => {
    if (typeof navigator === 'undefined' || !navigator.hid) {
        alert("Tu navegador no soporta WebHID. Por favor usa Google Chrome o Microsoft Edge en PC.");
        return;
    }
    try {
      ...
      device.addEventListener("inputreport", event => {
        ...
        // Inyección directa usando la memoria Ref (ahora sí funcionará el auto-avance)
      ...
      console.error(error);
      alert("Error USB. Verifica que el multímetro no esté abierto en tu programa de Python.");
    }
  };
  ...
  }, [campoActivoDock, mostrarAdmin, vistaAdmin]);
  ...
  // --- SOLUCIÓN DE PANTALLA DE CARGA (Para que Vercel no falle en el SSR) ---
  if (!pasoActual) {
    return (
          ...
          <div style={{display: 'flex', gap: '18px', alignItems: 'center'}}>
            ...
            {!mostrarAdmin && (
              <div 
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '30px', padding: '10px 25px', backgroundColor: '#0a0a0a', borderRadius: '10px', border: `2px solid ${usbConectado ? 'rgba(0, 255, 255, 0.4)' : '#374151'}`, cursor: 'pointer', boxShadow: usbConectado ? 'inset 0 0 15px rgba(0,255,255,0.1)' : 'none', minWidth: '150px', justifyContent: 'space-between' }} 
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <span style={{ fontSize: '2rem', color: '#6b7280', fontWeight: 'bold', marginTop: '2px', letterSpacing: '1px' }}>UT61E+</span>
                </div>
