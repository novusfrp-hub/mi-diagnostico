...
// --- FPC INTELIGENTE CON COLORES (ESTILO BORNEO/ZXW) ---
const FPCInteligente = ({ pines, setPines, pinActivo, setPinActivo, modo = 'diagnostico', escala = 'diodo', lecturaEnVivo }) => {
  ...
  const obtenerColorPin = (pin) => {
    // Colores Fijos de ZXW/Borneo
    if (pin.tipo === 'GND') return '#4b5563'; // Gris Tierra
    if (pin.tipo === 'NC') return '#1e3a8a';  // Azul Not Connected

    // Modo Crear (Visualización Base)
    if (modo === 'crear') {
        if (pinActivo === pin.id) return '#00ffff'; // Pin Seleccionado
        if (pin.tipo === 'VCC') return '#7f1d1d'; // Rojo oscuro para voltaje
        return '#374151'; // Gris Base para DATA
    }

    // Modo Diagnóstico (Comparador Matemático)
              ...
              {modo === 'crear' ? ( 
                <div style={{display:'flex', gap:'5px', marginTop:'5px'}}>
                    <input 
                      ...
                      style={{ background: '#000', color: 'white', border: '1px solid #333', padding: '5px', borderRadius: '5px', width: '160px', outline:'none', fontSize:'0.8rem' }} 
                    />
                    <select 
...
export default function AppDiagnostico() {
  ...
  const pinesIniciales = Array.from({ length: 40 }, (_, i) => ({ id: i + 1, nombre: `Linea_${i + 1}`, valorSano: '---', valorActual: '---', tipo: 'DATA' }));
    ...
    if (libreriaVisibleRef.current) {
        ...
        if (seccion === 'docktest') {
            ...
            setPinActivoFpc(prev => {
                // Lógica para saltar automáticamente los pines GND y NC
                let nextPin = prev + 1;
                while (nextPin <= fpcAct.pines.length) {
                    const pinInfo = fpcAct.pines.find(p => p.id === nextPin);
                    if (pinInfo && (pinInfo.tipo === 'GND' || pinInfo.tipo === 'NC')) { nextPin++; } 
                    else { break; }
                }
        ...
        setPinActivoFpc(prev => {
            let nextPin = prev + 1;
            while (nextPin <= fpcPines.length) {
                const pinInfo = fpcPines.find(p => p.id === nextPin);
                if (pinInfo && (pinInfo.tipo === 'GND' || pinInfo.tipo === 'NC')) nextPin++; else break;
            }
            return nextPin <= fpcPines.length ? nextPin : prev;
        });
    }
  };
  ...
  // --- SOLUCIÓN DE LA BARRA ESPACIADORA (LA CURA) ---
  useEffect(() => {
    const handleKeyDown = (e) => { 
        // Si el usuario está escribiendo en un input, IGNORAR la barra espaciadora global.
        if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return; 

        if ((e.code === 'Space' || e.key === 'Enter') && (libreriaVisible || mostrarAdmin)) { 
  ...
  const crearNuevoFpcEnModelo = () => { if(!formNuevoFpc.nombre || formNuevoFpc.pines <= 0) return; const pinesArray = Array.from({ length: parseInt(formNuevoFpc.pines) }, (_, i) => ({ id: i + 1, nombre: `Linea_${i + 1}`, valorSano: '---', valorActual: '---', tipo: 'DATA' })); const nuevoFpc = { id: Date.now().toString(), nombre: formNuevoFpc.nombre.replace(/\s+/g, '_'), pines: pinesArray }; const modActualizado = {...modeloActivo, fpcs: [...modeloActivo.fpcs, nuevoFpc]}; setModeloActivo(modActualizado); setFpcActivo(nuevoFpc); setFormNuevoFpc({ nombre: '', pines: 40 }); setPinActivoFpc(1); };
                            ...
                            <div>
                                <div style={{ display:'flex', gap:'15px', marginBottom:'20px' }}>
                                    ...
                                    <div style={{ display:'flex', gap:'5px', background:'#1a1a1a', padding:'5px', borderRadius:'10px', border:'1px solid #333' }}>
                                        <input placeholder="Nombre FPC..." value={formNuevoFpc.nombre} onChange={e=>setFormNuevoFpc({...formNuevoFpc, nombre: e.target.value.replace(/ /g, '_')})} style={{...estilos.inputDark, width:'120px', padding:'5px'}} />
                                    ...
                                    <>
                                        <div style={{display:'flex', justifyContent:'flex-end', gap:'10px', marginBottom:'15px'}}>
                                            <div style={{display:'flex', gap:'5px', backgroundColor: '#1a1a1a', padding:'4px', borderRadius:'8px', marginRight:'auto'}}>
                                                <button onClick={editarPinesFpcActivo} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', background: 'transparent', color: '#eab308', fontWeight: 'bold', cursor:'pointer' }}>✏️ Editar Pines</button>
                                                <button onClick={eliminarFpcActivo} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', background: 'transparent', color: '#ef4444', fontWeight: 'bold', cursor:'pointer' }}>🗑️ Eliminar FPC</button>
                                            </div>
