...
// Importación de tu nuevo motor de PDFs Profesionales
import { pdf } from '@react-pdf/renderer';
import ReportePDF from '../components/pdfx/ReportePDF';
...
export default function AppDiagnostico() {
  ...
  // ESTADOS NUEVOS: REPORTE PDF/WHATSAPP
  ...
  const imprimirReporte = async () => {
    ...
    try {
      // Usamos @react-pdf/renderer para crear un PDF nativo y descargable
  ...
  return (
    <div style={{ ...estilos.contenedor, ...t.fondoPrincipal }}>
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ ...estilos.modalCard, maxWidth: '800px', ...t.fondoPrincipal, ...t.bordeFantasma }}>
              ...
              <div style={{ ...estilos.modalBody, padding: '15px' }}>
                {casosGuardados.length === 0 ? (<div style={{ textAlign: 'center', padding: '40px', color: t.textoSutil.color }}><FileText size={48} style={{opacity: 0.5, marginBottom:'10px'}}/><p>No hay casos registrados aún.</p></div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>{casosGuardados.map(caso => (<div key={caso.id} style={{ backgroundColor: t.cristalBgItem.backgroundColor, border: t.bordeFantasma.border, borderRadius: '12px', padding: '15px', display: 'flex', flexDirection: 'column', gap: '10px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}><div><h4 style={{ margin: '0 0 5px 0', ...t.textoPrincipal, fontSize: '1.1rem' }}>{caso.marca} {caso.modelo}</h4><span style={{ fontSize: '0.75rem', color: t.textoSutil.color }}>Ingreso: {new Date(caso.fecha).toLocaleDateString()}</span></div><div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={() => abrirReporte(caso)} style={{ ...estilos.btnAccionLista, backgroundColor: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6', padding: '5px 12px', display: 'flex', gap: '5px', alignItems: 'center' }} title="Ver Reporte">
                    <FileText size={16} /> <span>Reporte</span>
                  </button>
      ...
      {/* MODAL: VISTA PREVIA DEL REPORTE Y WHATSAPP */}
      <AnimatePresence>
        {reporteVisible && casoReporte && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{...estilos.modalOverlay, backgroundColor: 'rgba(0,0,0,0.9)'}}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} style={{ width: '100%', maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center', height: '90vh' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', width: '100%', justifyContent: 'flex-end', padding: '10px 0' }}>
                <button onClick={enviarWhatsApp} style={{ background: '#25D366', color: 'white', padding: '10px 20px', borderRadius: '10px', border: 'none', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 10px rgba(37, 211, 102, 0.3)' }}>
                  <MessageCircle size={20} /> WhatsApp
                </button>
                <button onClick={imprimirReporte} style={{ background: '#0058bc', color: 'white', padding: '10px 20px', borderRadius: '10px', border: 'none', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 10px rgba(0, 88, 188, 0.3)' }}>
                  <Printer size={20} /> Imprimir / PDF
                </button>
                <button onClick={() => setReporteVisible(false)} style={{ background: '#374151', color: 'white', padding: '10px', borderRadius: '10px', border: 'none', cursor: 'pointer' }}>
                  <X size={20} />
                </button>
              </div>
              ...
              <div style={{ flex: 1, width: '100%', overflowY: 'auto', borderRadius: '8px' }}>
                <div id="seccion-reporte" style={{ backgroundColor: 'white', width: '100%', maxWidth: '210mm', minHeight: '297mm', margin: '0 auto', padding: '40px', boxSizing: 'border-box', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', color: '#111827', fontFamily: 'system-ui, sans-serif' }}>
                  ...
                  <div style={{ textAlign: 'center', borderBottom: '3px solid #0058bc', paddingBottom: '20px', marginBottom: '30px' }}>
                    <h1 style={{ color: '#0058bc', margin: 0, fontSize: '28px', textTransform: 'uppercase', letterSpacing: '2px' }}>MARSHALL CELL</h1>
                    <h2 style={{ color: '#4b5563', margin: '5px 0 0 0', fontSize: '16px', fontWeight: 'normal' }}>Reporte Técnico Oficial de Diagnóstico</h2>
                  </div>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                    <div style={{ background: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}><strong style={{ color: '#111827', display: 'block', marginBottom: '5px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Fecha de Ingreso</strong><p style={{ margin: 0, fontSize: '15px' }}>{new Date(casoReporte.fecha).toLocaleDateString()}</p></div>
                    <div style={{ background: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}><strong style={{ color: '#111827', display: 'block', marginBottom: '5px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>ID de Caso</strong><p style={{ margin: 0, fontSize: '15px' }}>#{casoReporte.id.substring(0, 8).toUpperCase()}</p></div>
                    <div style={{ background: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}><strong style={{ color: '#111827', display: 'block', marginBottom: '5px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Marca</strong><p style={{ margin: 0, fontSize: '15px' }}>{casoReporte.marca}</p></div>
                    <div style={{ background: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}><strong style={{ color: '#111827', display: 'block', marginBottom: '5px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Modelo</strong><p style={{ margin: 0, fontSize: '15px' }}>{casoReporte.modelo}</p></div>
                  </div>
                  
                  <div style={{ background: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb', marginBottom: '20px' }}>
                    <strong style={{ color: '#111827', display: 'block', marginBottom: '5px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Síntomas Reportados</strong>
                    <p style={{ margin: 0, fontSize: '15px', whiteSpace: 'pre-wrap', lineHeight: '1.5' }}>{casoReporte.sintomas}</p>
                  </div>
                  
                  <div style={{ background: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <strong style={{ color: '#111827', display: 'block', marginBottom: '5px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Protocolo de Reparación / Diagnóstico</strong>
                    <p style={{ margin: 0, fontSize: '15px', whiteSpace: 'pre-wrap', lineHeight: '1.5' }}>{casoReporte.protocolo || 'Pendiente de evaluación técnica profunda.'}</p>
                  </div>
                  
                  {/* AQUÍ ESTÁ EL TRUCO REFERRERPOLICY PARA EVITAR QUE SE ROMPA LA FOTO */}
                  {casoReporte.imgUrl && (
                  ...
                  <div style={{ marginTop: '50px', textAlign: 'center', fontSize: '12px', color: '#9ca3af', borderTop: '1px solid #e5e7eb', paddingTop: '20px' }}>
                    Generado por Marshall Cell CRM<br/>Laboratorio de Microelectrónica - Oropesa, Cusco, Perú
                  </div>
      ...
      {/* VISUALIZADORES MULTIMEDIA Y OTROS MODALES */}
