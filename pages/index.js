...
// Importaciones limpias: Solo los íconos que realmente usa tu app
import { Sun, Moon, ArrowLeft, RefreshCcw, Zap, Smartphone, AlertTriangle, ChevronRight, Home, ShieldCheck, Camera, CheckCircle2, XCircle, Settings, Plus, Save, X, Trash2, Edit, ChevronDown, CornerDownRight, LogOut, Lightbulb, Usb, Map, Youtube, Flame, ClipboardList, History, Printer, FileText } from 'lucide-react';
    ...
    ventana.document.write(`
      <html>
        ...
        <body>
          ...
          ${caso.imgUrl ? `<div class="img-container"><strong>Evidencia Técnica Adjunta</strong><br/><img src="${caso.imgUrl}" alt="Evidencia" /></div>` : ''}
          <div class="footer">Generado por Marshall Cell CRM<br/>Laboratorio de Microelectrónica - Oropesa, Cusco, Perú</div>
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ ...estilos.modalCard, ...t.fondoPrincipal, ...t.bordeFantasma }}>
              ...
              <div style={estilos.modalBody}>
                <form onSubmit={guardarBitacora} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  ...
                  <div><label style={{ ...estilos.labelForm, ...t.textoPrincipal }}>Síntomas *</label><textarea required style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, ...t.bordeFantasma, minHeight: '80px' }} value={formCaso.sintomas} onChange={(e) => setFormCaso({ ...formCaso, sintomas: e.target.value })} /></div>
                  <div><label style={{ ...estilos.labelForm, ...t.textoPrincipal }}>Protocolo / Diagnóstico</label><textarea style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, ...t.bordeFantasma, minHeight: '80px' }} value={formCaso.protocolo} onChange={(e) => setFormCaso({ ...formCaso, protocolo: e.target.value })} /></div>
                  <div><label style={{ ...estilos.labelForm, ...t.textoPrincipal }}>Enlace Imagen (Opcional)</label><input type="text" style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, ...t.bordeFantasma }} value={formCaso.imgUrl} onChange={(e) => setFormCaso({ ...formCaso, imgUrl: e.target.value })} /></div>
                  {mensajeCaso && <p style={{ color: mensajeCaso.includes('❌') ? '#ef4444' : '#10b981', fontWeight: 'bold', textAlign: 'center', margin: 0 }}>{mensajeCaso}</p>}
                  <button type="submit" style={{ ...estilos.btnPrimarioGuardar, background: '#10b981', justifyContent: 'center', padding: '15px' }}><Save size={20} style={{ marginRight: '8px' }} /> {casoEditando ? 'Actualizar' : 'Guardar'}</button>
                </form>
              </div>
            </motion.div>
          </motion.div>
