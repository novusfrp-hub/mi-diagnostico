...
export default function AppDiagnostico() {
  ...
  const [bitacoraVisible, setBitacoraVisible] = useState(false); const [historialCasosVisible, setHistorialCasosVisible] = useState(false);  const [casosGuardados, setCasosGuardados] = useState([]); const [casoEditando, setCasoEditando] = useState(null); const [formCaso, setFormCaso] = useState({ marca: '', modelo: '', tecnico: 'Marshall Cell', sintomas: '', protocolo: '', imgUrl: '', estadoReparacion: 'Pendiente', hardware: { cpu: '', memoria: '', pmic: '' }, consumoUsb: { voltaje: '', corriente: '', comportamiento: '', conBateria: '', sinBateria: '' }, consumoFuente: { inicial: '', postPower: '', comportamiento: '' }, lineasAfectadas: [] }); const [mensajeCaso, setMensajeCaso] = useState('');
