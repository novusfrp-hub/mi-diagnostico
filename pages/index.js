...
export default function AppDiagnostico() {
  ...
  const cargarPaso = async (idPaso, esRetroceso = false) => {
    ...
    try {
      ...
      setPasoActual(datos);
      setNotaVisible(false); setTipVisto(false); setMostrarTablaConsumos(false); setPanelMedicionVisible(false); setImgModalVisible(false); setVideoModalVisible(false);
  ...
  const exportarPDF = (caso) => {
    const ventana = window.open('', '_blank');
    ventana.document.write(`
      <html>
        <head>
          <title>Reporte de Diagnóstico - ${caso.marca} ${caso.modelo}</title>
          <style>
            body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 40px; color: #333; max-width: 800px; margin: 0 auto; }
            .header { text-align: center; border-bottom: 3px solid #0058bc; padding-bottom: 20px; margin-bottom: 30px; }
            h1 { color: #0058bc; margin: 0; font-size: 28px; text-transform: uppercase; letter-spacing: 2px; }
            h2 { color: #4b5563; margin: 5px 0 0 0; font-size: 16px; font-weight: normal; }
            .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px; }
            .box { background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #e5e7eb; }
            .box strong { color: #111827; display: block; margin-bottom: 5px; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; }
            .box p { margin: 0; font-size: 15px; line-height: 1.5; }
            .img-container { margin-top: 30px; text-align: center; background: #f9fafb; padding: 20px; border-radius: 8px; border: 1px solid #e5e7eb; }
            img { max-width: 100%; max-height: 350px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        ...
        <body>
          <div class="header">
            <h1>Marshall Cell</h1>
            <h2>Reporte Técnico Oficial de Diagnóstico</h2>
          </div>
          <div class="grid">
            <div class="box"><strong>Fecha de Ingreso</strong><p>${new Date(caso.fecha).toLocaleDateString()}</p></div>
            <div class="box"><strong>ID de Caso</strong><p>#${caso.id.substring(0, 8).toUpperCase()}</p></div>
            <div class="box"><strong>Marca</strong><p>${caso.marca}</p></div>
            <div class="box"><strong>Modelo</strong><p>${caso.modelo}</p></div>
          </div>
          ...
          <div class="box">
            <strong>Protocolo de Reparación / Diagnóstico</strong>
            <p>${caso.protocolo ? caso.protocolo.replace(/\n/g, '<br/>') : 'Pendiente de evaluación técnica profunda.'}</p>
          </div>
          ${caso.imgUrl ? `
          <div class="img-container">
            <strong>Evidencia Técnica Adjunta</strong><br/>
            <img src="${caso.imgUrl}" alt="Evidencia" />
          </div>` : ''}
          <div class="footer">
            Generado por Marshall Cell Diagnostics App<br/>
            Laboratorio de Microelectrónica - Oropesa, Cusco, Perú
          </div>
          <script>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', borderRadius: '10px', backgroundColor: t.cristalBgItem.backgroundColor, border: t.bordeFantasma.border }}>
          <div onClick={() => tieneHijos && toggleExpandir(idPaso)} style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: tieneHijos ? 'pointer' : 'default', flex: 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontWeight: 'bold', color: '#0058bc', fontSize: '0.9rem', display: 'flex', alignItems:'center', gap:'4px' }}>{paso.id} {paso.imgUrl && (paso.imgTipo === 'microscopio' ? <Camera size={12} color="#8b5cf6" /> : <Map size={12} color="#8b5cf6" />)} {paso.videoUrl && <Youtube size={12} color="#ef4444" />} {paso.esFallaEnSerie && <Flame size={12} color="#f97316" />}</span>
  ...
  return (
    <div style={{ ...estilos.contenedor, ...t.fondoPrincipal }}>
        ...
        <AnimatePresence>
          {pasoActual.id === 'inicio' && fallasEnSerie.length > 0 && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, height: 0 }} style={{ width: '100%', marginBottom: '30px' }}>
              <h3 style={{ ...t.textoPrincipal, fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '8px', margin: '0 0 15px 0' }}>
                <Flame size={20} color="#f97316" /> FALLAS CRÓNICAS (ACCESO RÁPIDO)
              </h3>
              <div style={estilos.carruselFallas}>
                {fallasEnSerie.map(falla => (
                  <motion.button key={falla.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} style={{ ...estilos.tarjetaFallaSerie, ...t.cristalBgItem, ...t.bordeFantasma }} onClick={() => cargarPaso(falla.id)}>
                    ...
                    <div style={{ display: 'flex', gap: '5px', marginTop: '10px' }}>{falla.imgUrl && <Camera size={14} color="#8b5cf6" />}{falla.videoUrl && <Youtube size={14} color="#ef4444" />}{(falla.docktestDiodo || falla.docktestUa) && <Usb size={14} color="#3b82f6" />}</div>
                  </motion.button>
                ))}
              </div>
            </motion.div>
          ...
          <motion.div key={pasoActual.id} initial={{ opacity: 0, scale: 0.98, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: -10 }} transition={{ duration: 0.4 }} style={{ ...estilos.tarjetaCristal, ...t.cristalBg, ...t.bordeFantasma }}>
            <div style={estilos.seccionTitulo}>
              <span style={{ ...estilos.etiquetaPaso, color: '#0058bc', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                {pasoActual.notaExperta && (<motion.button onClick={() => { setNotaVisible(true); setTipVisto(true); }} animate={!tipVisto ? { scale: [1, 1.1, 1], boxShadow: ["0px 0px 0px rgba(234, 179, 8, 0)", "0px 0px 15px rgba(234, 179, 8, 0.7)", "0px 0px 0px rgba(234, 179, 8, 0)"] } : { scale: 1, boxShadow: "none" }} transition={!tipVisto ? { duration: 1.5, repeat: Infinity, ease: "easeInOut" } : { duration: 0.3 }} style={estilos.btnBombillo} title="Tip del Experto"><Lightbulb size={16} color="#a16207" /> <span style={{fontSize: '0.75rem', fontWeight: '800', color: '#a16207'}}>TIP</span></motion.button>)}
                ...
                {pasoActual.videoUrl && (<button onClick={() => setVideoModalVisible(true)} style={{...estilos.btnImgFlotante, background: '#ef4444', boxShadow: '0 4px 10px rgba(239, 68, 68, 0.3)'}} title="Ver Video"><Youtube size={16} color="white" /> <span style={{fontSize: '0.7rem', fontWeight: '800', color: 'white'}}>VIDEO</span></button>)}
              </span>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={estilos.modalOverlay}>
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ ...estilos.modalCard, ...t.fondoPrincipal, ...t.bordeFantasma }}>
              <div style={estilos.modalHeader}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><ClipboardList size={24} color="#10b981" /><h3 style={{ margin: 0, ...t.textoPrincipal, fontSize: '1.2rem' }}>{casoEditando ? 'Editar Caso' : 'Nueva Bitácora de Ingreso'}</h3></div>
              ...
              <div style={estilos.modalBody}>
                <form onSubmit={guardarBitacora} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  ...
                  <div><label style={{...estilos.labelForm, ...t.textoPrincipal}}>Síntomas Reportados *</label><textarea required placeholder="El cliente indica que dejó de cargar tras usar un cargador genérico..." style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, ...t.bordeFantasma, minHeight: '80px' }} value={formCaso.sintomas} onChange={(e) => setFormCaso({...formCaso, sintomas: e.target.value})} /></div>
                  <div><label style={{...estilos.labelForm, ...t.textoPrincipal}}>Protocolo Seguido / Diagnóstico</label><textarea placeholder="1. Revisión visual. 2. Medición en escala de diodos VBUS. 3. Reemplazo de OVP..." style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, ...t.bordeFantasma, minHeight: '80px' }} value={formCaso.protocolo} onChange={(e) => setFormCaso({...formCaso, protocolo: e.target.value})} /></div>
                  <div><label style={{...estilos.labelForm, ...t.textoPrincipal}}>Enlace Imagen / Plano (Opcional)</label><input type="text" placeholder="Pega un enlace de postimages.org aquí..." style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, ...t.bordeFantasma }} value={formCaso.imgUrl} onChange={(e) => setFormCaso({...formCaso, imgUrl: e.target.value})} /></div>
                  ...
                  <button type="submit" style={{ ...estilos.btnPrimarioGuardar, background: '#10b981', justifyContent: 'center', padding: '15px' }}><Save size={20} style={{marginRight:'8px'}}/> {casoEditando ? 'Actualizar Caso' : 'Guardar en Historial'}</button>
                </form>
              </div>
            </motion.div>
          </motion.div>
        ...
        {historialCasosVisible && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={estilos.modalOverlay}>
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ ...estilos.modalCard, maxWidth: '800px', ...t.fondoPrincipal, ...t.bordeFantasma }}>
              <div style={estilos.modalHeader}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><History size={24} color="#0058bc" /><h3 style={{ margin: 0, ...t.textoPrincipal, fontSize: '1.2rem' }}>Historial de Reparaciones</h3></div>
                    ...
                    {casosGuardados.map(caso => (
                      <div key={caso.id} style={{ backgroundColor: t.cristalBgItem.backgroundColor, border: t.bordeFantasma.border, borderRadius: '12px', padding: '15px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <div>
                            <h4 style={{ margin: '0 0 5px 0', ...t.textoPrincipal, fontSize: '1.1rem' }}>{caso.marca} {caso.modelo}</h4>
                            <span style={{ fontSize: '0.75rem', color: t.textoSutil.color }}>Ingreso: {new Date(caso.fecha).toLocaleDateString()} | ID: {caso.id.substring(0,6)}</span>
                          </div>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            {/* BOTÓN MÁGICO DE PDF */}
                            <button onClick={() => exportarPDF(caso)} style={{ ...estilos.btnAccionLista, backgroundColor: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6' }} title="Generar PDF / Imprimir"><Printer size={18} /></button>
                            <button onClick={() => {setHistorialCasosVisible(false); editarCasoExistente(caso);}} style={{ ...estilos.btnAccionLista, backgroundColor: 'rgba(234, 179, 8, 0.1)', color: '#eab308' }} title="Editar"><Edit size={18} /></button>
                            <button onClick={() => eliminarCaso(caso.id)} style={{ ...estilos.btnAccionLista, backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#ef4444' }} title="Eliminar"><Trash2 size={18} /></button>
                          </div>
                        </div>
                        ...
                        {caso.protocolo && <div style={{ fontSize: '0.85rem', ...t.textoPrincipal }}><strong>Diagnóstico:</strong> {caso.protocolo.substring(0,100)}{caso.protocolo.length > 100 ? '...' : ''}</div>}
                      </div>
                    ))}
                  </div>
                )}
      ...
      {/* PANEL ADMIN OCULTO POR ESPACIO PERO FUNCIONAL */}
      <AnimatePresence>
        {mostrarAdmin && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={estilos.modalOverlay}>
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ ...estilos.modalCard, ...t.fondoPrincipal, ...t.bordeFantasma }}>
              ...
              {vistaAdmin === 'formulario' && (
                <>
                  <div style={estilos.modalBody}>
                    ...
                    <label style={{ ...estilos.labelForm, color: '#eab308', marginTop: '10px', display: 'flex', alignItems: 'center', gap: '5px' }}><Lightbulb size={16}/> Nota Experto</label><textarea style={{ ...estilos.inputForm, ...t.cristalBgItem, ...t.textoPrincipal, border: '1px solid rgba(234, 179, 8, 0.3)', minHeight: '40px' }} value={formNota} onChange={(e) => setFormNota(e.target.value)} />
